...
Attribute hWndToolTip.VB_Description = "Returns a handle to a control."
#End If
If ToolBarHandle <> NULL_PTR Then hWndToolTip = SendMessage(ToolBarHandle, TB_GETTOOLTIPS, 0, ByVal 0&)
End Property
...
ToolBarFontHandle = CreateGDIFontFromOLEFont(PropFont, PropFontQuality)
    ...
    If .lParam <> 0 Then
        ...
        Else
            ...
            If (.fsStyle And BTNS_WHOLEDROPDOWN) <> 0 Or ((.fsStyle And BTNS_DROPDOWN) <> 0 And (Shift And vbShiftMask) <> 0) Then
                ...
                ' Remove any left mouse button down or double-click messages so that we can get a toggle effect on the button.
                Dim Msg As TMSG
                Const PM_REMOVE As Long = &H1
                While PeekMessage(Msg, ToolBarHandle, WM_LBUTTONDOWN, WM_LBUTTONDOWN, PM_REMOVE) <> 0 Or PeekMessage(Msg, ToolBarHandle, WM_LBUTTONDBLCLK, WM_LBUTTONDBLCLK, PM_REMOVE) <> 0: Wend
            Else
                SendMessage ToolBarHandle, TB_PRESSBUTTON, ID, ByVal 1&
                UpdateWindow ToolBarHandle
                Sleep 50
